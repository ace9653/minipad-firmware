...
uint16_t KeypadHandler::readKey(const Key &key)
{
    ...
    // Perform an adc conversion if it has one
    else if ((key.externalADC > 0) && (key.type == KeyType::HallEffect) && (ADC_KEYS > 0))
    {

       uint16_t value = analogADCRead(HE_PIN(key.index), 1);
    ...
    //otherwise do standard hall effect
